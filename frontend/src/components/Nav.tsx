...
export default function Nav() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-[100] h-[var(--nav-height)] flex items-center justify-between px-10 transition-[background,border-color] duration-300 ease-in-out bg-white/70 backdrop-blur-md border-b border-white/40">
            ...
            >
                <LogoIcon size={40} />
                <span className="font-heading text-[30px] text-black leading-none">cohosted</span>
            </Link>
            ...
            <div className="flex items-center gap-3 flex-shrink-0 z-[1]">
                {loggedIn ? (
                    <>
                        <Link to="/events" className="font-sans text-[18px] font-medium text-[#444] no-underline cursor-pointer transition-colors duration-200 whitespace-nowrap hover:text-pink hover:no-underline">My Events</Link>
                        ...
                        <div className="relative" ref={profileRef}>
                            ...
                            {profileOpen && (
                                <div className="absolute top-[calc(100%+10px)] right-0 bg-white border border-border rounded-[var(--radius)] shadow-[var(--shadow)] min-w-[200px] py-2 z-[200]">
                                    <div className="px-4 pt-2 pb-[0.1rem] text-[0.88rem] font-semibold text-text-dark whitespace-nowrap overflow-hidden text-ellipsis">{displayName}</div>
                                    <div className="px-4 pb-2 text-[0.75rem] text-text-muted whitespace-nowrap overflow-hidden text-ellipsis">{profile?.email}</div>
            ...
            >
                <span className={`block w-6 h-[2px] bg-[#1a1a1a] rounded-[2px] transition-[transform,opacity] duration-[250ms] ease-in-out origin-center ${mobileOpen ? 'translate-y-[7px] rotate-45' : ''}`} />
                <span className={`block w-6 h-[2px] bg-[#1a1a1a] rounded-[2px] transition-[transform,opacity] duration-[250ms] ease-in-out origin-center ${mobileOpen ? 'opacity-0' : ''}`} />
                <span className={`block w-6 h-[2px] bg-[#1a1a1a] rounded-[2px] transition-[transform,opacity] duration-[250ms] ease-in-out origin-center ${mobileOpen ? '-translate-y-[7px] -rotate-45' : ''}`} />
            </button>
            ...
            {mobileOpen && (
                <div className="fixed top-[var(--nav-height)] left-0 right-0 bg-white/[0.97] backdrop-blur-[12px] border-b border-border flex flex-col px-8 pt-4 pb-6 gap-1 z-[99] shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
                    {loggedIn ? (
                        <>
                            <Link to="/events" className="font-sans text-[16px] font-medium text-[#333] no-underline py-3 border-b border-border-light cursor-pointer bg-transparent text-left transition-colors duration-200 hover:text-pink hover:no-underline" onClick={() => setMobileOpen(false)}>My Events</Link>
                            <Link to="/profile" className="font-sans text-[16px] font-medium text-[#333] no-underline py-3 border-b border-border-light cursor-pointer bg-transparent text-left transition-colors duration-200 hover:text-pink hover:no-underline" onClick={() => setMobileOpen(false)}>Edit Profile</Link>
                            <button className="font-sans text-[16px] font-medium text-danger bg-transparent border-none border-b-0 py-3 cursor-pointer text-left transition-colors duration-200" onClick={handleLogout}>Sign Out</button>
                        </>
                    ) : (
                        <>
                            <Link to="/how-it-works" className="font-sans text-[16px] font-medium text-[#333] no-underline py-3 border-b border-border-light cursor-pointer bg-transparent text-left transition-colors duration-200 hover:text-pink hover:no-underline" onClick={() => setMobileOpen(false)}>How It Works</Link>
                            <Link to="/templates" className="font-sans text-[16px] font-medium text-[#333] no-underline py-3 border-b border-border-light cursor-pointer bg-transparent text-left transition-colors duration-200 hover:text-pink hover:no-underline" onClick={() => setMobileOpen(false)}>Browse Templates</Link>
                            <Link to="/login" className="font-sans text-[16px] font-medium text-[#333] no-underline py-3 border-b border-border-light cursor-pointer bg-transparent text-left transition-colors duration-200 hover:text-pink hover:no-underline" onClick={() => setMobileOpen(false)}>Log In</Link>
                            <Link to="/signup" className="font-sans text-[16px] font-bold text-pink no-underline py-3 border-b-0 cursor-pointer bg-transparent text-left mt-2 transition-colors duration-200 hover:no-underline" onClick={() => setMobileOpen(false)}>Sign Up</Link>
                        </>
                    )}
                </div>
            )}
