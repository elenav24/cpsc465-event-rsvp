  ...
  return (
    <div className="ep-chat-panel">
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`ep-chat-msg${mine ? " mine" : ""}`}>
              <div className="ep-chat-avatar">
                {getInitials(msg.sender_name)}
              </div>
              <div className="ep-chat-bubble">
                <div className="ep-chat-name">
      ...
      <div className="ep-chat-input-row">
        ...
        />
        <button
          className="ep-send-btn ep-send-btn--pink"
          onClick={send}
          aria-label="Send"
        >
        ...
        return (
          <div key={poll.id} className="ep-card">
            <div className="ep-poll-question">
              {poll.question}
              {poll.is_closed && (
                <span className="ep-badge ep-badge--gray">Closed</span>
              )}
              {poll.is_anonymous && (
                <span className="ep-badge ep-badge--purple">Anonymous</span>
              )}
            </div>
            ...
            {poll.options.map((opt) => {
              ...
              return (
                <div
                  key={opt.id}
                  className={`ep-poll-option${voted ? " voted" : ""}`}
                  onClick={() =>
                    !poll.is_closed &&
                    handleVote(
                      poll.id,
            ...
            <div className="ep-meta" style={{ marginTop: 6 }}>
            ...
            {isHost && !poll.is_closed && (
              <div className="ep-row-actions">
                <button
                  ...
                  className="ep-btn-ghost ep-btn-ghost--sm"
                >
                  Close poll
                </button>
                <button
                  onClick={() => handleDelete(poll.id)}
                  className="ep-btn-danger-ghost ep-btn-ghost--sm"
                >
                  Delete
                </button>
              </div>
                ...
                <div className="ep-meta">
                  ...
                  {isAssignedToMe
                    ...
                    : members.find((m) => m.user_id === task.assigned_to)
                        ?.display_name || task.assigned_to.slice(0, 8) + "…"}
                </div>
              )}
              {task.due_date && (
                <div className="ep-meta">
  ...
  return (
    <div className="ep-tab-body">
      ...
      {isHost && (
        ...
        >
          <textarea
            className="ep-field ep-textarea"
      ...
      {announcements.length === 0 && (
        <div className="ep-tab-empty">No announcements yet.</div>
      )}
        ...
        <div key={ann.id} className="ep-card ep-card--announcement">
          ...
          >
            <div className="ep-meta">
              ...
              {ann.sms_sent && (
                <span style={{ marginLeft: 8, color: "var(--pink)" }}>
                  SMS sent
                </span>
              )}
            </div>
            {(isHost || ann.author_id === myId) && (
              <button
                ...
                className="ep-btn-danger-ghost"
                style={{ fontSize: "0.72rem", padding: "2px 0" }}
              >
                Delete
              </button>
            )}
  ...
  return (
    <div className="ep-ai-panel">
      ...
      <div className="ep-ai-messages">
        {messages.length === 0 && !loading && (
          <div className="ep-ai-empty">
            ...
            <div>
              Ask anything about this event —<br />
              guests, polls, tasks, or the chat.
            </div>
          </div>
        )}
        {messages.map((msg, i) => (
          <div
            key={i}
            className={`ep-ai-msg${msg.role === "user" ? " user" : " assistant"}`}
          >
            <div className="ep-ai-msg-avatar">
      ...
      <div className="ep-ai-input-row">
        ...
        />
        <button
          className="ep-send-btn ep-send-btn--purple"
          onClick={send}
          disabled={loading}
          aria-label="Send"
        >
  ...
  return (
    <div className="ep-edit-panel">
      <div className="ep-edit-header">
        ...
        <button onClick={onCancel} className="ep-edit-close" aria-label="Close">
          &times;
        </button>
      </div>
      {error && <div className="ep-tab-error">{error}</div>}
      <form onSubmit={handleSubmit} className="ep-edit-form">
        ...
        >
          <div>
            <label className="ep-field-label">Start Date</label>
            <input
              className="ep-field"
              type="date"
              value={startDate}
          ...
          <div>
            <label className="ep-field-label">Start Time</label>
            <input
              className="ep-field"
              type="time"
              value={startTime}
        ...
        >
          <div>
            <label className="ep-field-label">End Date</label>
            <input
              className="ep-field"
              type="date"
              value={endDate}
          ...
          <div>
            <label className="ep-field-label">End Time</label>
            <input
              className="ep-field"
              type="time"
              value={endTime}
        ...
        <label className="ep-checkbox-label" style={{ marginTop: 4 }}>
          <input
            type="checkbox"
            checked={viewableByLink}
            onChange={(e) => setViewableByLink(e.target.checked)}
          />
          <span>Viewable by anyone with the link</span>
        </label>
        <div className="ep-form-actions" style={{ marginTop: 8 }}>
          <button type="button" onClick={onCancel} className="ep-btn-ghost">
            Cancel
          </button>
          <button type="submit" disabled={saving} className="ep-btn-primary">
  ...
  if (error && !event)
    return (
      ...
      >
        ...
        <button onClick={() => navigate("/events")} className="ep-btn-primary">
          Back to Events
        </button>
      </div>
    );
  ...
  return (
    <div className="ep-root">
        ...
        <div className="ep-sidebar-inner">
            ...
            <div className="ep-info-card-body">
              ...
              {event.start_dt && (
                <div className="ep-info-row">
                  ...
                  <span className="ep-info-val">
                    ...
                    {event.end_dt ? ` – ${formatDate(event.end_dt)}` : ""}
                  </span>
                </div>
              )}
              {event.recurrence_rule && (
                <div className="ep-info-row">
                  ...
                  <span className="ep-info-val">
                    {event.recurrence_rule.charAt(0) +
                      event.recurrence_rule.slice(1).toLowerCase()}
                    {event.recurrence_end_dt
                      ? ` until ${new Date(event.recurrence_end_dt).toLocaleDateString()}`
                      : ""}
                  </span>
                </div>
              )}
              ...
              {event.viewable_by_link && (
                <div className="ep-info-row" style={{ color: "var(--pink)" }}>
                  <FiLink
                    size={13}
                    className="ep-info-icon"
                    style={{ color: "var(--pink)" }}
                  />
                  <span
                    className="ep-info-val"
                    style={{ color: "var(--pink)" }}
                  >
          ...
          <div className="ep-info-card">
            <div className="ep-info-card-header ep-info-card-header--green">
              <div className="ep-info-label ep-info-label--green">
                YOUR RSVP
              </div>
            </div>
            <div className="ep-info-card-body">
              ...
              {(myRsvp?.status === "yes" || myRsvp?.status === "maybe") && (
                <div className="ep-guest-count-row">
                  ...
                  <div className="ep-guest-count-ctrl">
                    <button
                      type="button"
                      className="ep-guest-count-btn"
                      onClick={() => {
                        const n = Math.max(0, guestCount - 1);
                        setGuestCount(n);
                        handleRsvp(myRsvp!.status, n);
                      }}
          ...
          {isHost && (
            <div className="ep-info-card">
              <div className="ep-info-card-header ep-info-card-header--orange">
                <div className="ep-info-label ep-info-label--orange">
                  INVITE LINK
                </div>
              </div>
              <div className="ep-info-card-body">
                {event.invite_token && event.invite_active ? (
                  <>
                    <div className="ep-invite-row">
                      ...
                      <button
                        ...
                        className="ep-invite-eye"
                        title={inviteLinkVisible ? "Hide" : "Show"}
                      >
                        {inviteLinkVisible ? (
                          <svg
                            width="13"
                      ...
                      <button
                        ...
                        className={`ep-invite-copy${inviteCopied ? " copied" : ""}`}
                      >
                        {inviteCopied ? (
                          <>
                    ...
                    <div className="ep-invite-actions">
                      ...
                      <button
                        ...
                        className="ep-link-btn ep-link-btn--danger"
                      >
                        Disable
                      </button>
                      <button
                  ...
                  <>
                    <div
                      className="ep-meta"
                      style={{ color: "#c00", marginBottom: 6 }}
                    >
                      Link is disabled.
                    </div>
                    <button
          ...
          <div className="ep-info-card">
            <div className="ep-info-card-header ep-info-card-header--rose">
              <div className="ep-info-label ep-info-label--rose">
                GUESTS ({eventMembers.length})
              </div>
            </div>
            <div className="ep-info-card-body">
              <div className="ep-guest-list">
                {eventMembers.slice(0, 10).map((m) => {
                  ...
                  return (
                    <div key={m.id} className="ep-guest-row">
                      ...
                      {rsvp && (
                        <span
                          className={`ep-guest-rsvp ep-guest-rsvp--${rsvp.status}`}
                        >
                          {rsvp.status === "yes" ? (
                            <FiCheck size={10} />
                          ) : rsvp.status === "no" ? (
                        ...
                        <div className="ep-guest-actions">
                          {isCoHost ? (
                            <button
                              ...
                              onClick={() =>
                                handleRoleChange(m.user_id, "attendee")
                              }
                            >
                              co-host ↓
                            </button>
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleRoleChange(m.user_id, "co_host")
                              }
                            >
                              + co-host
                            </button>
                ...
                {eventMembers.length > 10 && (
                  <div className="ep-meta" style={{ marginTop: 4 }}>
                    +{eventMembers.length - 10} more
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="ep-main">
        ...
        {error && (
          <div className="ep-tab-error" style={{ margin: "0.75rem 1rem 0" }}>
            {error}
          </div>
        )}
        ...
        <div className="ep-tab-content">
          {activeTab === "polls" && (
            <PollsTab
              ...
              myId={myId}
              isHost={isHost}
              version={resourceVersions["poll"] ?? 0}
            />
          )}
          {activeTab === "tasks" && (
            <TasksTab
              ...
              version={resourceVersions["task"] ?? 0}
              onTasksChange={setEventTasks}
            />
          )}
          {activeTab === "potluck" && (
            <PotluckTab
              ...
              myId={myId}
              isHost={isHost}
              version={resourceVersions["potluck"] ?? 0}
            />
          )}
          {activeTab === "announcements" && (
            <AnnouncementsTab
              ...
              myId={myId}
              isHost={isHost}
              onNew={() => markUnread("announcements")}
              version={resourceVersions["announcement"] ?? 0}
            />
          )}
          {activeTab === "ai" && (
            <AiTab
