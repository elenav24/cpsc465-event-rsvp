...
class Event(Base):
    ...
    recurrence_rule: Mapped[Optional[str]] = mapped_column(String, nullable=True)  # e.g. "WEEKLY", "DAILY", "MONTHLY"
    recurrence_end_dt: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    parent_event_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("events.id"), nullable=True)

    # Invite link — all events are invite-only
    invite_token: Mapped[Optional[str]] = mapped_column(String, unique=True, nullable=True, index=True)
    ...
    viewable_by_link: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    polls: Mapped[list["Poll"]] = relationship("Poll", back_populates="event", cascade="all, delete-orphan")
    potluck_items: Mapped[list["PotluckItem"]] = relationship("PotluckItem", back_populates="event", cascade="all, delete-orphan")
    ...
    pending_invites: Mapped[list["PendingInvite"]] = relationship("PendingInvite", back_populates="event", cascade="all, delete-orphan")
