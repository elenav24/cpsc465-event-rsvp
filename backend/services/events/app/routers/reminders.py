...
def create_reminder(
    ...
    if not event.start_dt:
        raise HTTPException(
            status_code=400,
            detail="Event has no start time set — cannot schedule reminder",
        )
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Reminder with this offset already exists"
        )
...
@router.delete(
    "/{event_uuid}/reminders/{reminder_id}", status_code=status.HTTP_204_NO_CONTENT
)
def delete_reminder(
    event_uuid: str,
    reminder_id: int,
