...
def test_rsvp_yes(client):
    event_id = client.post("/events", data={"title": "E"}).json()["id"]
    res = client.put(f"/events/{event_id}/rsvps", json={"status": "yes", "guest_count": 2})
...
def test_rsvp_update(client):
    event_id = client.post("/events", data={"title": "E"}).json()["id"]
    client.put(f"/events/{event_id}/rsvps", json={"status": "yes"})
    res = client.put(f"/events/{event_id}/rsvps", json={"status": "maybe"})
    assert res.json()["status"] == "maybe"


def test_invalid_rsvp_status(client):
    event_id = client.post("/events", data={"title": "E"}).json()["id"]
    res = client.put(f"/events/{event_id}/rsvps", json={"status": "definitely"})
...
def test_rsvp_no_releases_potluck_claim(client, set_user):
    ...
    set_user(TEST_USER_SUB)
    item = client.post(f"/events/{event_id}/potluck", json={
    ...
    set_user(OTHER_USER_SUB)
    client.put(f"/events/{event_id}/rsvps", json={"status": "yes"})
    client.post(f"/events/{event_id}/potluck/{item['id']}/claim")

    # Verify claim exists
    items = client.get(f"/events/{event_id}/potluck").json()
    assert items[0]["claims_count"] == 1

    # Other user changes RSVP to no — claim should be released
    client.put(f"/events/{event_id}/rsvps", json={"status": "no"})
    items = client.get(f"/events/{event_id}/potluck").json()
...
def test_non_member_cannot_rsvp(client, set_user):
    ...
    event_id = client.post("/events", data={"title": "E"}).json()["id"]
    set_user(OTHER_USER_SUB)
    res = client.put(f"/events/{event_id}/rsvps", json={"status": "yes"})
