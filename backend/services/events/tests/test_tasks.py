...
def test_host_creates_task(client, set_user):
    ...
    set_user(TEST_USER_SUB)
    res = client.post(f"/events/{event_id}/tasks", json={"title": "Book venue"})
...
def test_attendee_cannot_create_task(client, set_user):
    ...
    res = client.post(f"/events/{event_id}/tasks", json={"title": "Sneaky task"})
...
def test_attendee_can_volunteer(client, set_user):
    event_id = _setup(client, set_user)
    set_user(TEST_USER_SUB)
    task = client.post(f"/events/{event_id}/tasks", json={"title": "Bring ice"}).json()
    set_user(OTHER_USER_SUB)
    res = client.put(f"/events/{event_id}/tasks/{task['id']}",
...
def test_assigned_member_can_complete(client, set_user):
    event_id = _setup(client, set_user)
    set_user(TEST_USER_SUB)
    task = client.post(f"/events/{event_id}/tasks", json={
        "title": "Get cups",
...
def test_unassigned_member_cannot_complete(client, set_user):
    event_id = _setup(client, set_user)
    set_user(TEST_USER_SUB)
    task = client.post(f"/events/{event_id}/tasks", json={"title": "Unassigned"}).json()
    set_user(OTHER_USER_SUB)
    res = client.put(f"/events/{event_id}/tasks/{task['id']}",
                     json={"is_completed": True})
    assert res.status_code == 403
